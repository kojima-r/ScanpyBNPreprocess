...
```
  ...
  02resample.py --level <L>                  02pseudo_bulk.py --level <L>
   <src>=r                                    <src>=p
        │                                          │
        ▼                                          ▼
  02data_bbknn_r_<L>/                       02data_bbknn_p_<L>/
        │                                          │
        ├── 連続値経路 (col-merge / ingor) ────────┤
        │                                          │
        ▼ 03transpose --source <src> --level <L>   ▼
  02data_bbknn_<src>_<L>_t/                  02data_bbknn_<src>_<L>_t/
        │                                          │
        ▼ 04merge --source <src> --level <L>       ▼
  03data_bbknn_<src>_<L>/all.txt             03data_bbknn_<src>_<L>/all.txt
                          │
                          ▼  ┌─[ ingor ]─┐
                             │ 05run.sh  │
                             │ 06run_ecv │
        ...
        └── 離散化経路 (row-merge / disc, src=r & L=batch を推奨) ──┐
                                                                    ▼
                              04merge --batched --source r --level batch
                              03data_bbknn_b_r_batch/all.txt
                                            │
                                            ▼ 05disc.py
                              03data_bbknn_b_r_batch/all_disc.txt
                              03data_bbknn_b_r_batch/all_disc_tri.txt
                                            │
                                            ▼ 06prep_disc.py
                              03data_bbknn_b_r_batch/all_disc{,10,100,1000}.tsv
                              03data_bbknn_b_r_batch/tissue/<tissue>.tsv
                                            │
                                            ▼
                              ┌─────────────────────────────┐
...
出力先の既定値は `02data_bbknn_r_<level>/`、1 列目は `s<i>|<層別キー>` の形。

| `--level` | 層別キー (1 列目)                              | 既定の出力先              |
| --------- | ---------------------------------------------- | ------------------------- |
| `tissue`  | `s<i>\|<tissue>` (各臓器内で 1 プール)         | `02data_bbknn_r_tissue/`  |
| `age`     | `s<i>\|<tissue>\|<age>`                        | `02data_bbknn_r_age/`     |
| `batch`   | `s<i>\|<tissue>\|<age>\|<batch>`               | `02data_bbknn_r_batch/`   |

```sh
python 02resample.py --input-glob "01data_bbknn/*.txt" --level tissue -n 10
python 02resample.py --input-glob "01data_bbknn/*.txt" --level age    -n 10
python 02resample.py --input-glob "01data_bbknn/*.txt" --level batch  -n 10  # 離散化系バックエンド向け
```

**出力例 (`--level tissue`, `-n 2`):** `02data_bbknn_r_tissue/Aorta.txt`
...
**出力例 (`--level age`, `-n 2`):** `02data_bbknn_r_age/Aorta.txt`
...
**出力例 (`--level batch`, `-n 2`):** `02data_bbknn_r_batch/Aorta.txt`
...
出力先は `02data_bbknn_p_<level>/`、1 列目は集約キーそのもの。

| `--level` | 集約キー (1 列目)                     | 既定の出力先              |
| --------- | ------------------------------------- | ------------------------- |
| `tissue`  | `<tissue>` (1 行 / ファイル)          | `02data_bbknn_p_tissue/`   |
| `age`     | `<tissue>\|<age>`                     | `02data_bbknn_p_age/`      |
| `batch`   | `<tissue>\|<age>\|<batch>`            | `02data_bbknn_p_batch/`    |

```sh
python 02pseudo_bulk.py --input-glob "01data_bbknn/*.txt" --level tissue
python 02pseudo_bulk.py --input-glob "01data_bbknn/*.txt" --level age
python 02pseudo_bulk.py --input-glob "01data_bbknn/*.txt" --level batch
```

**出力例 (`--level tissue`):** `02data_bbknn_p_tissue/Aorta.txt` (1 行)
...
**出力例 (`--level age`):** `02data_bbknn_p_age/Aorta.txt`
...
**出力例 (`--level batch`):** `02data_bbknn_p_batch/Aorta.txt`
...
#   02data_bbknn_r_tissue/*  →  02data_bbknn_r_tissue_t/

# pseudo_bulk (--level age) を転置
python 03transpose.py --source p --level age
#   02data_bbknn_p_age/*  →  02data_bbknn_p_age_t/
```

**入力例:** `02data_bbknn_r_tissue/Aorta.txt` (上記 §2 の出力, `samples × genes`)
**出力例:** `02data_bbknn_r_tissue_t/Aorta.txt` (`genes × samples`)
...
`--source` / `--level` で入出力が自動決定されます。

```sh
# resample (--level tissue) を結合
python 04merge.py --source r --level tissue
#   02data_bbknn_r_tissue_t/*  →  03data_bbknn_r_tissue/all.txt

# pseudo_bulk (--level age) を結合
python 04merge.py --source p --level age
#   02data_bbknn_p_age_t/*  →  03data_bbknn_p_age/all.txt
```

**入力例:** `02data_bbknn_r_tissue_t/Aorta.txt`, `02data_bbknn_r_tissue_t/BAT.txt`, ... (§3 の出力, `genes × samples`)
**出力例:** `03data_bbknn_r_tissue/all.txt` (列名は `<tissue>.txt_<sample>`)
...
離散化系では `--level batch` で層別したリサンプル (`02data_bbknn_r_batch/`) を行方向に結合し、各細胞の (tissue, age, batch) を残します。

```sh
python 04merge.py --batched --source r --level batch
#   02data_bbknn_r_batch/*  →  03data_bbknn_b_r_batch/all.txt
```

**入力例:** `02data_bbknn_r_batch/Aorta.txt`, `02data_bbknn_r_batch/BAT.txt`, ... (§2 --level batch の出力)
**出力例:** `03data_bbknn_b_r_batch/all.txt` (末尾に `tissue` 列)
...
**入力例:** `03data_bbknn_b_r_batch/all.txt` (§4 --batched の出力)
**出力例:** `03data_bbknn_b_r_batch/all_disc.txt` (binary, age が 0 埋めされ並び替えされる)

```
@name                Sox17  Mrpl15  Msc  Rdh10  ...    tissue
s0|Aorta|03m|1       0      1       1    1      ...    Aorta
s0|Aorta|18m|1       1      1       1    1      ...    Aorta
s0|BAT|03m|1         1      1       0    1      ...    BAT
s0|Brain_Myeloid|03m|1   0  1       1    1      ...    Brain_Myeloid
...
```

**出力例:** `03data_bbknn_b_r_batch/all_disc_tri.txt` (ternary, 値は 0/1/2)

```
@name                Sox17  Mrpl15  Msc  Rdh10  ...    tissue
s0|Aorta|03m|1       0      2       1    2      ...    Aorta
...
**入力例:** `03data_bbknn_b_r_batch/all_disc.txt`, `03data_bbknn_b_r_batch/all_disc_tri.txt` (§5 の出力)

**出力例 (すべて `03data_bbknn_b_r_batch/` 配下):**
...
**サンプル:** `03data_bbknn_b_r_batch/all_disc100.tsv` (header 行 + 0/1 のみ、`@name`/`tissue` 列なし)
...
`03data_bbknn_<src>_<L>/all.txt` (転置・連続値) を入力として、`ingor` のブートストラップ構造学習を行います。

```sh
sh 05run.sh 03data_bbknn_r_tissue/all.txt   # §4 の出力 (resample --level tissue)
sh 05run.sh 03data_bbknn_p_tissue/all.txt   # §4 の出力 (pseudo_bulk --level tissue)
sh 05run_each.sh                            # 各臓器ごとに個別に推定
```

**入力例:** `03data_bbknn_r_tissue/all.txt` (§4 の出力, `genes × samples`, 連続値)
...
ブートストラップ後、ECv で全体ネットワークと臓器ごとの個別ネットワークを得ます (`06run_ecv_all.sh` の `tag` 引数で `bs_all<tag>/` と `02data_bbknn<tag>_t/` を選びます。新パスを使う場合は同名のディレクトリを揃えるか `tag` を空にしておきます):
...
C++ 実装の Hill-Climb + Tabu (BIC/BDeu/K2)。`06prep_disc.py` が生成した `03data_bbknn_b_r_batch/all_disc{,1000}.tsv` を直接読み込みます。

```sh
cd FastBN
sh compile.sh                      # g++ -O3 -std=c++17 fast_bn.cpp -o fast_bn
sh run.sh                          # → 03data_bbknn_b_r_batch/all_disc1000.tsv で構造学習
sh pred.sh                         # 学習済み構造に対する再スコア + edge-importance
```

**入力例:** `03data_bbknn_b_r_batch/all_disc1000.tsv` (§6 の出力, 0/1 表)
**出力例 (`FastBN/` 配下):**
...
```sh
cd MatDNF
sh run.sh                # = python learn_bn_matdnf.py ../03data_bbknn_b_r_batch/all_disc100.tsv
```

**入力例:** `03data_bbknn_b_r_batch/all_disc100.tsv` (§6 の出力)
**出力例:** `03data_bbknn_b_r_batch/all_disc100_matdnf.json`
...
```sh
cd ilp_bn
sh run.sh                # = python bn_ilp_pulp.py ../03data_bbknn_b_r_batch/all_disc100.tsv
```

**入力例:** `03data_bbknn_b_r_batch/all_disc100.tsv` (§6 の出力)
**出力例:** `03data_bbknn_b_r_batch/all_disc100_bn_structure.json`
...
```sh
cd pgmpy
sh run.sh                # = python learn_bn_pgmpy.py ../03data_bbknn_b_r_batch/all_disc100.tsv (hybrid+k2)
```
...
| `02data_bbknn_r_<L>/`               | resample 出力                                           |
| `02data_bbknn_p_<L>/`               | pseudo_bulk 出力                                        |
| `02data_bbknn_<src>_<L>_t/`         | 上記の転置 (genes×cells)                              |
| `03data_bbknn_<src>_<L>/all.txt`    | 列方向結合 — ingor 入口 (連続値)                       |
| `03data_bbknn_b_<src>_<L>/all.txt`  | 行方向結合 (`--batched`) — 離散化前の元データ           |
| `03data_bbknn_b_<src>_<L>/all_disc{,_tri}.txt`        | 離散化後 (binary / ternary)        |
| `03data_bbknn_b_<src>_<L>/all_disc{,10,100,1000}.tsv` | 離散化系 4 出口の共通入力          |
| `03data_bbknn_b_<src>_<L>/tissue[_tri]/`              | 臓器別 0/1 (or 0/1/2) 表           |
